...
export class OptimizedBookPage implements AfterViewInit{
  ...
  constructor(public navCtrl: NavController,public platform: Platform,public companies: CompaniesProvider) {
	  ...
	  this.nextImage = this.imageList[this.pageNum+2];
  ...
  ngAfterViewInit(){
	...
	this.PAGE_WIDTH = this.BOOK_WIDTH/2-this.PAGE_MARGIN ;
...
drawFlip( flip ) {
		...
		this.actualRightPage.nativeElement.style.width = Math.max(foldX - foldWidth , 0) + "px";	
